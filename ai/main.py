...
async def analizar_imagen(file: UploadFile = File(...)):
    ...
    if modelo is not None:
        try:
            ...
            # Recalibrar: el modelo sobreestima defectos por desbalance del dataset
            # Solo marca deterioro cuando el modelo está muy seguro (>85% confianza)
            prob_defecto = max(0, (prob_defecto - 0.85) / 0.15)
            porcentaje_deterioro = round(prob_defecto * 100, 2)

            print(f"🔍 prob_defecto recalibrada: {prob_defecto:.4f} → {porcentaje_deterioro}%")
